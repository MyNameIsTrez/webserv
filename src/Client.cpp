...
void Client::appendReadString(char *received, ssize_t bytes_read)
{
    if (this->client_to_server_state == ClientToServerState::HEADER)
    {
        ...
        if (!this->_fillHeaders(header_lines, this->headers))
...
void Client::respondWithFile(const std::string &path)
{
    ...
    static std::unordered_map<std::string, std::string> extension_to_content_type = {
        ...
        {".log", "text/plain"}, {".md", "text/plain"},         {".mk", "text/plain"}, {".py", "text/plain"},
        {".tpp", "text/plain"}, {".txt", "text/plain"},

        {".html", "text/html"}, {".mp3", "audio/mpeg"},        {".png", "image/png"}, {".pdf", "application/pdf"},
        {".gif", "image/gif"},  {".json", "application/json"}, {".mp4", "video/mp4"},
    };
...
void Client::respondWithDirectoryListing(const std::string &path)
{
    ...
    {
        this->response += "<a href=\"" + directory_name + "\">" + directory_name + "</a>\n";
    }

    for (const std::string &non_directory_name : _getSortedEntryNames(path, false))
    {
        this->response += "<a href=\"" + non_directory_name + "\">" + non_directory_name + "</a>\n";
    }
...
{
    return (this->protocol.size() == std::string("HTTP/0.0").size()) && (Utils::startsWith(this->protocol, "HTTP/")) &&
           (this->protocol[5] >= '0' && this->protocol[5] <= '9') && (this->protocol[6] == '.') &&
           (this->protocol[7] >= '0' || this->protocol[7] <= '9');
}

bool Client::_fillHeaders(const std::vector<std::string> &header_lines,
                          std::unordered_map<std::string, std::string> &filled_headers)
{
    for (const std::string &line : header_lines)
    {
        ...
        if (filled_headers.contains(key))
            return false;
...
void Client::_useHeaders(void)
{
    ...
    if (content_length_it != this->headers.end())
    {
        ...
        // that contains a Transfer-Encoding header field. (http1.1 rfc 6.2)
        if (this->_is_chunked)
            throw ClientException(Status::BAD_REQUEST);

        if (!Utils::parseNumber(content_length_it->second, this->content_length, std::numeric_limits<size_t>::max()))
            throw ClientException(Status::BAD_REQUEST);
    ...
    {
        // "A client MUST send a Host header field" - HTTP/1.1 RFC 9112,
        // section 3.2 Request Target
...
void Client::_parseBodyAppend(const std::string &extra_body)
{
    if (this->_is_chunked)
    {
        ...
        while (true)
        {
            ...
            {
                // Check if current buffer has anything else than a hexadecimal
                // character
            ...
            if (this->_chunked_read_state == ChunkedReadState::READING_CONTENT_LEN_ENDLINE ||
                this->_chunked_read_state == ChunkedReadState::READING_BODY_ENDLINE)
            {
                ...
                if (this->_chunked_body_buffer[0] == '\r' && this->_chunked_body_buffer[1] == '\n')
                {
                    ...
                    if (this->_chunked_remaining_content_length == 0 &&
                        this->_chunked_read_state == ChunkedReadState::READING_CONTENT_LEN_ENDLINE)
                    {
                        this->client_to_server_state = ClientToServerState::DONE;
                    }
...
std::string Client::_getFileName(const std::string &path)
{
    ...
    if (slash_index == path.npos || slash_index + 1 >= path.length())
        return "";
