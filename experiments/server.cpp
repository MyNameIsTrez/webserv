...
// c++ server.cpp -Wall -Wextra -Werror -Wpedantic -Wshadow -Wfatal-errors -g -fsanitize=address,undefined && ./a.out
...
int main(void)
{
    ...
    while (true)
    {
        ...
        for (nfds_t j = 0; j < nfds; j++)
        {
            ...
            {
                printf("  fd=%d; events: %s%s%s\n", poll_fds[j].fd, (poll_fds[j].revents & POLLIN) ? "POLLIN " : "",
